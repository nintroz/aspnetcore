    ...
    public sealed class ComplexObjectModelBinder : IModelBinder
    {
        ...
        private async Task BindModelCoreAsync(ModelBindingContext bindingContext, int propertyData)
        {
            ...
            if (bindingContext.Model == null)
            {
                ...
                var values = ParameterDefaultValues.GetParameterDefaultValues(boundConstructor.Identity.ConstructorInfo);

                var (attemptedParameterBinding, parameterBindingSucceeded) = await BindItems(
                    bindingContext,
                    propertyData,
            ...
            var (attemptedPropertyBinding, propertyBindingSucceeded) = await BindItems(
                bindingContext, 
                propertyData, 
        ...
        private async ValueTask<(bool attemptedBinding, bool bindingSucceeded)> BindItems(
            ModelBindingContext bindingContext, 
            int propertyData, 
            IReadOnlyList<ModelMetadata> items,
            ...
            object[] parameterValues)
        {
            ...
            {
                var itemMetadata = items[i];
                if (!CanBindItem(bindingContext, itemMetadata))
                {
                    continue;
                }

                var binder = bindingParameter ? _parameterBinders[i] : _propertyBinders[itemMetadata];
                ...
                var result = await BindItem(bindingContext, itemMetadata, binder);
                ...
                else if (itemMetadata.IsBindingRequired)
                {
                    attemptedBinding = true;
                }
            }
            ...
            if (postponePlaceholderBinding && bindingSucceeded)
            {
                ...
                for (var i = 0; i < items.Count; i++)
                {
                    ...
                    var itemBinder = bindingParameter ? _parameterBinders[i] : _propertyBinders[itemMetadata];
                    if (itemBinder is PlaceholderBinder)
                    {
                        var result = await BindItem(bindingContext, itemMetadata, itemBinder);
            ...
            Task<ModelBindingResult> BindItem(ModelBindingContext bindingContext, ModelMetadata itemMetadata, IModelBinder itemBinder)
            {
                if (bindingParameter)
                {
                    var fieldName = itemMetadata.BinderModelName ?? itemMetadata.ParameterName;
                    var modelName = ModelNames.CreatePropertyModelName(bindingContext.ModelName, fieldName);
                    return BindParameter(bindingContext, itemMetadata, itemBinder, fieldName, modelName);
                }
                else
                {
                    var fieldName = itemMetadata.BinderModelName ?? itemMetadata.PropertyName;
                    var modelName = ModelNames.CreatePropertyModelName(bindingContext.ModelName, fieldName);
                    return BindProperty(bindingContext, itemMetadata, itemBinder, fieldName, modelName);
                }
            }
        ...
        internal int CanCreateModel(ModelBindingContext bindingContext)
        {
            ...
            return CanBindAnyModelItems(bindingContext);
        }

        private int CanBindAnyModelItems(ModelBindingContext bindingContext)
        {
                ...
                case ModelMetadataKind.Parameter:
                    throw new InvalidOperationException(
                        Resources.FormatComplexObjectModelBinder_NoSuitableConstructor_ForParameter(
                            modelType.FullName,
                            nameof(ModelBindingConstructor),
